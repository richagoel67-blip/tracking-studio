...
export function QuickTestLiveEventStream({
  rows,
  listening,
  ended,
  onShareResults,
  ...
  return (
    <div className="flex h-full min-h-[420px] flex-col rounded-lg border border-slate-800 bg-slate-950 shadow-inner">
      <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-100">Live Event Stream</h2>
        <div className="flex items-center gap-2">
          <span
            className={cn(
              "size-2 rounded-full",
              listening && !ended ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.7)]" : "bg-slate-500",
            )}
            aria-hidden
          />
          <span className="text-xs font-medium text-slate-300">
            {ended ? "Stopped" : listening ? "Listening" : "Idle"}
          </span>
        </div>
      </div>
          ...
          <ul className="space-y-1.5">
            {rows.map((r) => (
              <li key={r.id} className="flex flex-wrap gap-x-2 gap-y-0.5 border-b border-slate-800/80 pb-1.5 last:border-0">
                ...
                <span className="min-w-0 text-slate-400">{r.jtrack}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="flex flex-col gap-2 border-t border-slate-800 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-slate-400">
          <span className="font-semibold text-slate-200">{rows.length}</span> events captured
        </p>
        <Button
          type="button"
          size="sm"
